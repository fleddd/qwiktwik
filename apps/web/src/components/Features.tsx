...
export default function Features() {
    // Масив залишив тільки з необхідними даними для лицьової сторони
    const features = [
        {
    ...
    const containerVariants = {
        ...
        visible: {
            opacity: 1,
            transition: {
                staggerChildren: 0.15 // Картки з'являються по черзі
            }
        }
    };

    // Анімація появи кожної картки
                ...
                >
                    {features.map((f, i) => (
                        <motion.div key={i} variants={itemVariants} className="h-full">
                            ...
                            {/* Статична картка */}
                            <div className="bg-charcoal border border-white/5 rounded-2xl h-full flex flex-col p-6 md:p-8 shadow-xl relative overflow-hidden">

                                <div className="flex justify-between items-start mb-auto">
                                    <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center shrink-0 shadow-[0_0_15px_rgba(0,255,102,0.1)]">
