...
const ADMIN_MENU_ITEMS = [
    ...
    { id: 'admin-affiliates', path: '/dashboard/admin/affiliates', label: 'Affiliates', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
...
export default function Sidebar({ user }: { user: UserProfileResponse }) {
    ...
    // Динамічно формуємо меню користувача залежно від ролі
    const USER_MENU_ITEMS = useMemo(() => {
        const items = [...BASE_USER_MENU_ITEMS];
        // Додаємо Partner Dashboard, якщо юзер - афіліат або адмін
        if (user.role === 'AFFILIATE' || user.role === 'ADMIN') {
            // Вставляємо після "Overview" (індекс 1)
            items.splice(1, 0, {
                id: 'aff-overview',
    ...
    // Вибір активного меню (Адмін або Звичайний юзер)
    const activeMenu = (isAdminRoute && user.role === 'ADMIN') ? ADMIN_MENU_ITEMS : USER_MENU_ITEMS;
    ...
    return (
        <>
            <div className="md:hidden flex items-center justify-between p-5 bg-[#0a0a0c] border-b border-white/5 sticky top-0 z-40">
                ...
                <button onClick={() => setIsMobileMenuOpen(true)} className="text-white hover:text-accent transition-colors">
            ...
            {isMobileMenuOpen && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-40 md:hidden touch-none" onClick={() => setIsMobileMenuOpen(false)} />
            )}
            ...
            `}>
                ...
                {/* Блок перемикання ТІЛЬКИ для Адміна */}
                {user.role === 'ADMIN' && (
                    <div className="mb-4 pb-4 border-b border-white/5 shrink-0">
                        <Link href={isAdminRoute ? '/dashboard' : '/dashboard/admin'} className="flex items-center justify-center gap-2 w-full px-4 py-2 bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded-lg text-[10px] font-bold uppercase tracking-wider hover:bg-purple-500/20">
                ...
                <nav className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-2 no-scrollbar pb-2">
                    {activeMenu.map((item) => {
                        const isActive = (item.path === '/dashboard' || item.path === '/dashboard/admin' || item.path === '/dashboard/affiliate')
                            ? pathname === item.path
                            : pathname.startsWith(item.path);

                        return (
                            <Link key={item.id} href={item.path} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${isActive ? 'bg-accent text-black shadow-glow' : 'text-text-muted hover:bg-white/5 hover:text-white'}`}>
