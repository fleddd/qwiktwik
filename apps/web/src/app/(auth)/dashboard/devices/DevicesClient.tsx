...
export default function DevicesClient({ initialDevices }: { initialDevices: DeviceResponse[] }) {
    ...
    if (devices.length === 0) {
        return (
            <div className="bg-charcoal border border-white/5 p-12 rounded-3xl text-center flex flex-col items-center">
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {devices.map((device) => (
                <div key={device.id} className="bg-charcoal border border-white/5 p-6 rounded-2xl flex flex-col">
