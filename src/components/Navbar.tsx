...
export default function Navbar() {
    ...
    const navLinks = [
        { label: 'Features', href: `${PAGES.HOME}#features` },
        { label: 'Modes', href: `${PAGES.HOME}#modes` },
        { label: 'Pro Version', href: `${PAGES.HOME}#pro` },
        { label: 'Docs', href: PAGES.DOCS, highlight: true },
        { label: 'FAQ', href: `${PAGES.HOME}#faq` },
    ];
            ...
            <nav className="fixed top-0 w-full py-4 md:py-5 bg-[#050505]/95 backdrop-blur-xl border-b border-white/5 z-[2000]">
                <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
                    ...
                    <ul className="hidden md:flex gap-8">
                        {navLinks.map((link) => (
                            <li key={link.label}>
                                <Link
                                    href={link.href}
                                    className={`text-sm font-bold transition-colors ${link.highlight ? 'text-[#00FF66] hover:text-[#00d957]' : 'text-[#94a3b8] hover:text-white'
                                        }`}
                                >
                                    {link.label}
                                </Link>
                    ...
                    <div className="hidden md:flex items-center gap-4">
                        ...
                        <Link href={`${PAGES.PRICING_FREE}#`} className="px-5 py-2.5 bg-[#00FF66] text-black font-black rounded-lg shadow-[0_4px_20px_rgba(0,255,102,0.2)] hover:bg-[#00d957] transition-all text-sm">
                            Download Free
                        </Link>
                    </div>

                    {/* Кнопка бургера / Х (завжди зверху) */}
                    <button
                                ...
                                {navLinks.map((link, i) => (
                                    ...
                                    >
                                        <Link
                                            href={link.href}
                                            onClick={() => setIsOpen(false)}
                                            className={`text-4xl font-black ${link.highlight ? 'text-[#00FF66]' : 'text-white'
                                                }`}
                                        >
                                            {link.label}
                                        </Link>
